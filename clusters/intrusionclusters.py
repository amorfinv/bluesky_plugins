...
class Clustering(core.Entity):
    ...
    def apply_density_rules(self, polygons, edges_df):
        ...
        # anything above this cutoff is high density and below is medium
    ...
    def apply_density_rules_easy(self, polygons, edges_df):
      ...
      # when there are two use 1 low and 1 high
            ...
            polygons['density_category'] = polygons['los_linear_density'].apply(lambda x: 'low' if x == polygons['los_linear_density'].min() else 'high')
        

        # when there are three use 1 medium and 2 high
        elif len(polygons) == 3:
            # Identify the minimum, median, and maximum density values
            min_density = polygons['los_linear_density'].min()
            median_density = polygons['los_linear_density'].median()
            max_density = polygons['los_linear_density'].max()

            # Assign labels based on density comparisons using np.where
            polygons['density_category'] = np.where(polygons['los_linear_density'] == min_density, 'low',
                                                    np.where(polygons['los_linear_density'] == median_density, 'medium', 'high'))
    ...
    def calc_densities(self, polygons, new_edges):
        ...
        polygons['los_linear_density'] = polygons['los_count'] / polygons['edge_length'] * 1000
